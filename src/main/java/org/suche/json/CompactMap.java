...
public final class CompactMap extends AbstractMap<String, Object> implements ContextBacked, JSONObject {
	...
	public CompactMap(final int initialKeys) {
		this.singleType = PRIMITIVE.T_EMPTY;
	...
	@Override public boolean isNull(final String pKey) {
		...
		case PRIMITIVE.T_LONG   -> false;
		case PRIMITIVE.T_DOUBLE -> false;
	...
	@Override public JSONArray getJSONArray(final String pKey) {
		final var idx = idx(pKey);
		if(idx < 0) throw new ClassCastException("Key '" + pKey + "' not found. KEYS: "+keySet());
		final var val = rawValueAt(idx);
		if(val instanceof final JSONArray l) return l;
		if(val == null) throw new ClassCastException("Value for key '" + pKey + "' is not a JSONArray but NULL");
		throw new ClassCastException("Value for key '" + pKey + "' is not a JSONArray but "+val.getClass().getCanonicalName());
	}
	...
	@Override public String optString(final String pKey, final String fallback) {
		...
		case PRIMITIVE.T_LONG   -> Long  .toString(                        prims[idx] );
		case PRIMITIVE.T_DOUBLE -> Double.toString(Double.longBitsToDouble(prims[idx]));
		default                -> {
			final var val = rawValueAt(idx);
	...
	private Object resolve(final int idx) {
		return switch(singleType) {
		case PRIMITIVE.T_EMPTY  -> null;
		case PRIMITIVE.T_LONG   -> Long  .valueOf(                        prims[idx] );
	...
	private int idx(final String key) {
		if(singleType == PRIMITIVE.T_EMPTY || data == null) return -1;
	...
	private void upgradeToMixed() {
		...
		case PRIMITIVE.T_LONG   -> { for (var i = 0; i < l; i += 2) if (d[i] != null) d[i + 1] = PRIMITIVE.LONG;   }
		case PRIMITIVE.T_DOUBLE -> { for (var i = 0; i < l; i += 2) if (d[i] != null) d[i + 1] = PRIMITIVE.DOUBLE; }
		default                -> { break; }
		}
	...
	private int getOrCreateIdx(final String key) {
		...
		for (var i = 0; i < data.length; i += 2) {
			...
			else if (key == data[i] || key.equals(data[i]))  return i >> 1;
		}
		if (freeIdx >= 0) {
			data[freeIdx << 1] = key;
			return freeIdx;
	...
	private long[] ensurePrims(final int idx) {
		if (prims == null) prims = new long[idx+1];  else if(idx >= prims.length) prims = Arrays.copyOf(prims, idx+1);
		return prims;
	}

	@Override public JSONObject put(final String key, final long val) {
		if (singleType == PRIMITIVE.T_EMPTY) singleType = PRIMITIVE.T_LONG;
		final var idx = getOrCreateIdx(key);
		ensurePrims(idx);
		switch (singleType) {
		case PRIMITIVE.T_LONG   -> prims[idx] = val;
		case PRIMITIVE.T_DOUBLE -> prims[idx] = Double.doubleToRawLongBits(val);
		default -> { // MIXED
	...
	@Override public JSONObject put(final String key, final double val) {
		if (singleType == PRIMITIVE.T_EMPTY) singleType = PRIMITIVE.T_DOUBLE;
		final var idx = getOrCreateIdx(key);
		if (singleType == PRIMITIVE.T_LONG && prims != null) upgradeToDouble();
		ensurePrims(idx)[idx] = Double.doubleToRawLongBits(val);
		return this;
	}
